...
CMD:healme(playerid,parmas[])
{
	// Add the function that will check if the Player is NOT VIP level 2
	if(!IsPlayerVipType(playerid, 2)) return SendClientMessage(playerid, COLOR_WHITE, "ERROR: You need VIP Level 2 to use this command!");
	{
		SetPlayerHealth(playerid, 100);
	}
	return 1;
